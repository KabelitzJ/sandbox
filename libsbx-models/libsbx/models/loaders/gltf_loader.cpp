...
static auto process_node(const aiNode* node, const aiScene* scene, mesh::mesh_data& data) -> void {
  for (auto i = 0u; i < node->mNumMeshes; i++) {
    const auto mesh_index = node->mMeshes[i];
    const auto* mesh = scene->mMeshes[mesh_index];
    process_mesh(mesh, data);
  }
...
static auto process_mesh(const aiMesh* mesh, mesh::mesh_data& data) -> void {
    ...
    //         glm::vec3(mesh->mTangents[i].x, mesh->mTangents[i].y, mesh->mTangents[i].z) : glm::vec3(0.0f);
    ...
    // Store the submesh (vertices + indices) in your engine data structures
}
...
auto gltf_loader::load(const std::filesystem::path& path) -> mesh::mesh_data {
  ...
  for (auto i = 0u; i < scene->mNumMeshes; ++i) {
    ...
    if (!mesh->HasNormals() || !mesh->HasTextureCoords(0) || !mesh->HasTangentsAndBitangents()) {
      throw std::runtime_error{fmt::format("Mesh '{}' does not have required attributes", mesh->mName.C_Str())};
    }

    for (auto j = 0u; j < mesh->mNumVertices; ++j) {
      const auto position = aivec3_to_vector3(mesh->mVertices[j]);
      const auto normal = aivec3_to_vector3(mesh->mNormals[j]);
      const auto uv = aivec3_to_vector3(mesh->mTextureCoords[0][j]);
      const auto tangent = aivec3_to_vector3(mesh->mTangents[j]);

      data.vertices.emplace_back(position, normal, uv, tangent);
    ...
    const auto old_index_count = data.indices.size();
    ...
    submesh.index_count = static_cast<std::uint32_t>(data.indices.size() - old_index_count);
...
// auto _old_load(const std::filesystem::path& path) -> mesh::mesh_data {
//   auto data = mesh::mesh_data{};
