...
auto image2d_array::create_descriptor_set_layout_binding(std::uint32_t binding, VkDescriptorType descriptor_type, VkShaderStageFlags shader_stage_flags) noexcept -> VkDescriptorSetLayoutBinding {
...
auto image2d_array::write_descriptor_set(std::uint32_t binding, VkDescriptorType descriptor_type) const noexcept -> graphics::write_descriptor_set {
  ...
  for (const auto id : _image_ids) {
    ...
    descriptor_image_info.sampler = image.sampler();
...
auto image2d_array::push_back(const handle_type& handle) -> std::uint32_t {
