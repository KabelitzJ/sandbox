...
struct canvas_world_push {
  graphics::buffer::address_type vertex_address;
  std::uint32_t sampler_index;
}; // struct canvas_world_push

canvas_world_pass::canvas_world_pass() {
  ...
  _text_pipeline = pipeline_cache.get(graphics::graphics_pipeline::create_info{
    ...
    .samples = render_pass::sample_count,
    .color_blend_attachments = {blend},
    .name = "Canvas World Text"
  });
}

auto canvas_world_pass::declare(graphics_pass_builder& builder, const graph_resources& resources) -> void {
...
auto canvas_world_pass::should_execute(const render_context& context, std::uint32_t /*group*/) const -> bool {
  auto& canvas_module = core::engine::get_module<canvas::canvas_module>();

  auto& draw_list = canvas_module.draw_list();

  return context.packet->camera.is_active && (!draw_list.world_vertices().empty() || !draw_list.world_glyph_vertices().empty());
}

auto canvas_world_pass::_draw(render_context& context, graphics::graphics_pipeline& pipeline, std::array<graphics::buffer_handle, graphics::swapchain::max_frames_in_flight>& buffers, std::array<std::size_t, graphics::swapchain::max_frames_in_flight>& capacities, const std::vector<canvas::canvas_vertex>& vertices) -> void {
  if (vertices.empty()) {
    return;
  }
  ...
  write_push_constants(context, canvas_world_push{buffer.address(), context.sampler_index});

  context.command_buffer->draw(static_cast<std::uint32_t>(vertices.size()), 1u, 0u, 0u);
}

auto canvas_world_pass::execute(render_context& context, std::uint32_t /*group*/) -> void {
  SBX_PROFILE_SCOPE("canvas_world_pass::execute");
  SBX_PROFILE_GPU_SCOPE((*context.command_buffer), "canvas_world_pass::execute");
  ...
  _draw(context, *_pipeline, _buffers, _capacities, draw_list.world_vertices());
  _draw(context, *_text_pipeline, _glyph_buffers, _glyph_capacities, draw_list.world_glyph_vertices());
}
