...
struct std::hash<sbx::math::basic_vector2<Type>> {
  std::size_t operator()(const sbx::math::basic_vector2<Type>& vector) const noexcept;
}; // struct std::hash

template<sbx::math::numeric Type>
struct YAML::convert<sbx::math::basic_vector2<Type>> {
  static Node encode(const sbx::math::basic_vector2<Type>& vector);
  static bool decode(const Node& node, sbx::math::basic_vector2<Type>& vector);
}; // struct YAML::convert
