...
//   direction = clamp(direction * rcpDirMin, -FXAA_SPAN_MAX, FXAA_SPAN_MAX) * texel;
// 
//   vec3 rgbA = 0.5 * (texture(in_image, in_uv + direction * (1.0 / 3.0 - 0.5)).rgb + texture(in_image, in_uv + direction * (2.0 / 3.0 - 0.5)).rgb);
//   vec3 rgbB = rgbA * 0.5 + 0.25 * (texture(in_image, in_uv + direction * -0.5).rgb + texture(in_image, in_uv + direction * 0.5).rgb);
// 
...
void main() {
  ...
  vec2 min_direction = min(vec2(8.0, 8.0), max(vec2(-8.0, -8.0), direction * rcp_direction_min)) * inv_resolution;

  vec3 rgb_a = 0.5 * (texture(in_image, in_uv + min_direction * (1.0 / 3.0 - 0.5)).xyz + texture(in_image, in_uv + min_direction * (2.0 / 3.0 - 0.5)).xyz);

  vec3 rgb_b = rgb_a * 0.5 + 0.25 * (texture(in_image, in_uv + min_direction * -0.5).xyz + texture(in_image, in_uv + min_direction * 0.5).xyz);

  float luma_b = dot(rgb_b, luma);

  out_color = ((luma_b < luma_min) || (luma_b > luma_max)) ? vec4(rgb_a, 1.0) : vec4(rgb_b, 1.0);
}
