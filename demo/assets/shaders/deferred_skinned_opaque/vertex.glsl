...
void main() {
  ...
  vec3 T = normalize(vec3(transform_data.normal * vec4(in_tangent.xyz, 0.0)));
  vec3 N = normalize(vec3(transform_data.normal * vec4(skinned_normal, 0.0)));
  vec3 B = cross(T, N) * in_tangent.w; // w: sign of the tangent
