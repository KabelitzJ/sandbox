...
layout(set = 0, binding = 0) uniform camera_buffer_object {
  mat4 view;
  mat4 proj;
} camera;
...
layout(set = 2, binding = 0) buffer in_blades { blade data[]; } buffer_in_blades;

layout(set = 2, binding = 1) buffer culled_blades { blade data[]; } buffer_culled_blades;
...
//
layout(set = 2, binding = 2) buffer num_blades {
...
void main() {
  ...
  if (gl_GlobalInvocationID.x == 0) {
    buffer_num_blades.vertexCount = 0;
  }
  ...
  //
  blade blade = buffer_in_blades.data[gl_GlobalInvocationID.x];
  ...
  blade.v1.xyz = v1;
  blade.v2.xyz = v2;
  buffer_in_blades.data[gl_GlobalInvocationID.x] = blade;
  ...
  // 3. Distance Test
  vec3 c = vec3(inverse(camera.view) * vec4(0.0, 0.0, 0.0, 1.0));  // camera position
  float d_proj = length(v0 - c - up * dot(v0 - c, up));
  const float d_max = 15.0;
  const int num_buckets = 20;
  bool culled_by_distance = ((gl_GlobalInvocationID.x % num_buckets) > floor(num_buckets * (1 - d_proj / d_max)));

  // final output
  if (!culled_by_distance && !culledByOrientation && !culledByViewFrustum) {
    buffer_culled_blades.data[atomicAdd(buffer_num_blades.vertexCount, 1)] = blade;
  }
}
